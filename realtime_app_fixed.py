...
try:
    ...
    print("⚠️ VaderSentiment not available, using fallback")
...
# Initialize Whisper with smaller model and error handling
whisper_model = None
if WHISPER_AVAILABLE:
    try:
        # Try tiny model first (uses less memory)
        whisper_model = WhisperModel("tiny", compute_type="int8")
        print("✅ Whisper tiny model loaded successfully")
    except Exception as e:
        print(f"❌ Failed to load Whisper tiny model: {e}")
        try:
            # Fallback to base model with different settings
            whisper_model = WhisperModel("base", compute_type="float32", num_workers=1)
            print("✅ Whisper base model loaded with fallback settings")
        except Exception as e2:
            print(f"❌ Failed to load Whisper base model: {e2}")
            whisper_model = None
            ...
            if self.vad.is_speech(audio_array):
                if whisper_model:
                    try:
                        ...
                        return "Transcription temporarily unavailable", True
                else:
                    # Fallback when Whisper is not available
                    return "Speech detected (Whisper not available)", True
...
def handle_stop_recording():
    ...
    try:
        ...
        if summarizer and len(full_text.split()) > 10:
            try:
...
if __name__ == '__main__':
    ...
    print(f"   Summarizer: {'✅ Available' if summarizer else '❌ Not available'}")
    ...
    try:
        socketio.run(app, host='0.0.0.0', port=5000, debug=False)  # Disable debug to reduce memory usage
    except Exception as e:
